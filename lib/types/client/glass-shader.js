...
const FS_SRC = `
  ...
  void main() {
    ...
    if (hitBgLens == 1) {
      underlyingColor = calculateLensColor(bestBgCenter, bestBgHalf, bestBgRadius, minBgD, fragPx, uv, bgFlowOffset, 1.0, 0);
    } else {
      float shadow = 0.0;
    ...
    // =========================================================================
    // 2. 模态对话框 (设置窗口 / 手机连接二维码弹窗 / 确认框) 全屏与透镜物理虚化
    // =========================================================================
    if (u_has_modal == 1 && u_modal_progress > 0.001) {
      // 2.1 全屏底层场景高斯虚化与暗化
      vec3 fullSceneBlurred = sampleGaussianFrosted(finalBgUv, u_modal_blur, fragPx);
      if (u_l1_opacity > 0.001) {
        fullSceneBlurred = mix(fullSceneBlurred, vec3(0.04, 0.07, 0.12), clamp(u_l1_opacity, 0.0, 0.95));
      }
      underlyingColor = mix(underlyingColor, fullSceneBlurred, u_modal_progress);

      // 2.2 模态对话框几何包围盒内部处理（边缘微光与倒角）
      float modalDist = sdRoundedBox(fragPx - u_modal_rect.xy, u_modal_rect.zw, u_modal_radius);
      if (modalDist <= 0.0) {
        if (hitFgLens == 1) {
          vec3 fgLensColor = calculateLensColor(bestFgCenter, bestFgHalf, bestFgRadius, minFgD, fragPx, uv, bgFlowOffset, u_modal_progress, 1);
          underlyingColor = mix(underlyingColor, fgLensColor, u_modal_progress);
        } else if (hitBgLens == 1) {
          vec2 pBg = fragPx - bestBgCenter;
          vec2 normPos = clamp(pBg / max(bestBgHalf, vec2(1.0)), -1.0, 1.0);
          vec2 internalBulge = normPos * (1.0 - length(normPos) * 0.35) * 0.35 * u_bulge;
          vec2 modalSampleUv = clamp(uv + internalBulge * max(u_ior - 1.0, 0.08) * 1.6 + bgFlowOffset, 0.001, 0.999);
          vec3 modalBulgeBg = sampleGaussianFrosted(modalSampleUv, u_modal_blur, fragPx);
          if (u_l1_opacity > 0.001) {
            modalBulgeBg = mix(modalBulgeBg, vec3(0.04, 0.07, 0.12), clamp(u_l1_opacity, 0.0, 0.95));
          }
          underlyingColor = mix(underlyingColor, modalBulgeBg, u_modal_progress);
        }

        if (u_l1_border > 0.001 && abs(modalDist) <= 2.5) {
          float glint = smoothstep(2.5, 0.0, abs(modalDist)) * u_l1_border;
          underlyingColor = mix(underlyingColor, vec3(0.92, 0.96, 1.0), glint * u_modal_progress);
        }
      }
    }
...
`;
export function attachLiquidGlassShader(canvas, currentOpts) {
    ...
    function loadWallpaper(url) {
        if (url === currentWallpaperUrl && (customImg || customVideo))
            return;
    ...
    function frame(now) {
        ...
        try {
            ...
            // 0. 探测 Layer 1 顶层胶囊按钮与展开下拉菜单视窗 (节流查询，由 WebGL 专属执行 16-Tap 高斯雾化采样)
            if (lensScanFrameCounter % 2 === 0) {
                cachedPopoverElements = Array.from(document.querySelectorAll('[data-dsh-model-menu], [data-dsh-model-trigger], [class*="ModelSelect_trigger"], [class*="PermissionSelect_trigger"], button:has([data-ogr-auto-glyph]), [data-dsh-hero-workspace], [class*="HeroShell_workspace"], [class*="heroShell_workspace"], [data-dsh-hero-preset], [class*="AgentPresetSeat_seat"], [class*="agentPresetSeat_seat"], [class*="WorkspacePicker_trigger"], [class*="PresetMenu_button"], [role="menu"]:not([class*="PluginCard"]):not([class*="plugin"]), [class*="Menu_list"]:not([class*="PluginCard"]), [class*="ModelSelect_menu"], [class*="modelSelect_menu"], [class*="MenuView_menu"], [class*="PopupSelectView_card"], [class*="AgentPresetSeat_popover"], [class*="WorkspacePicker_popover"], [class*="PresetMenu_menu"], [data-radix-popper-content-wrapper] > div'));
            }
            let popoverCount = 0;
            popoverBuffer.fill(0);
            popoverRadiiBuffer.fill(0);
            for (let i = 0; i < cachedPopoverElements.length && popoverCount < 16; i++) {
                const pEl = cachedPopoverElements[i];
                if (!pEl || (pEl.offsetWidth === 0 && pEl.offsetHeight === 0))
                    continue;
                const pRect = pEl.getBoundingClientRect();
                if (pRect.width < 10 || pRect.height < 10 || pRect.bottom <= 0 || pRect.top >= screenH)
                    continue;
                const classStr = typeof pEl.className === 'string' ? pEl.className : (typeof pEl.className?.baseVal === 'string' ? pEl.className.baseVal : '');
                const isPopoverClosing = classStr.includes('closing') || classStr.includes('Closing') || pEl.parentElement?.className?.includes?.('closing');
                if (isPopoverClosing)
                    continue;
                // 绝不扫描插件中心或菜单内部子项
                if (pEl.closest('[class*="Plugin"], [class*="plugin"], [class*="Marketplace"], [class*="marketplace"]') !== null) {
                    continue;
                }
                // 绝不扫描处于展开菜单内部的子按钮/选项项，避免菜单背景分裂成横向条纹
                const isMenuRoot = pEl.getAttribute('role') === 'menu' || classStr.includes('menu') || classStr.includes('Menu') || classStr.includes('list');
                if (!isMenuRoot && pEl.closest('[role="menu"], [class*="Menu_list"], [class*="ModelSelect_menu"], [class*="modelSelect_menu"]') !== null) {
                    continue;
                }
                const pCenterX = (pRect.left + pRect.width * 0.5) * dpr;
                const pCenterY = (screenH - (pRect.top + pRect.height * 0.5)) * dpr;
                const pHalfW = (pRect.width * 0.5) * dpr;
                const pHalfH = (pRect.height * 0.5) * dpr;
                const isCapsule = pRect.height <= 36 && !isMenuRoot;
                const pRadius = isCapsule ? pHalfH : Math.min(20 * dpr, pHalfH);
                popoverBuffer[popoverCount * 4 + 0] = pCenterX;
                popoverBuffer[popoverCount * 4 + 1] = pCenterY;
                popoverBuffer[popoverCount * 4 + 2] = pHalfW;
                popoverBuffer[popoverCount * 4 + 3] = pHalfH;
                popoverRadiiBuffer[popoverCount] = pRadius;
                popoverCount++;
            }
            gl.uniform4fv(uPopoversLoc, popoverBuffer);
            gl.uniform1fv(uPopoverRadiiLoc, popoverRadiiBuffer);
            gl.uniform1i(uPopoverCountLoc, popoverCount);
            ...
            // 1.1 探测 Layer 1 (模态弹窗/设置面板/手机连接二维码弹窗) 几何尺寸
            const candidates = document.querySelectorAll('[data-dsh-settings-modal], [data-dsh-modal-panel], [class*="SettingsRoot_panel"], [class*="RemotePanel_panel"], [role="dialog"][aria-modal="true"], [class*="Modal_panel"], [class*="modal_panel"], [class*="remote"], [class*="Remote"]');
            let modalEl = null;
            let maxArea = 0;
            for (let i = 0; i < candidates.length; i++) {
                const el = candidates[i];
                const rect = el.getBoundingClientRect();
                if (rect.width > 40 && rect.height > 40 && rect.bottom > 0 && rect.top < screenH) {
                    const area = rect.width * rect.height;
                    if (el.hasAttribute('data-dsh-settings-modal') || el.hasAttribute('data-dsh-modal-panel') || area > maxArea) {
                        maxArea = area;
                        modalEl = el;
                        if (el.hasAttribute('data-dsh-settings-modal') || el.hasAttribute('data-dsh-modal-panel'))
                            break;
                    }
                }
            }
            ...
            if (modalEl && modalEl.offsetWidth > 0 && modalEl.offsetHeight > 0) {
                ...
                const isClosing = modalEl.getAttribute('data-closing') === 'true' ||
                    ...
                    modalEl.parentElement?.className?.includes?.('Closing');
                if (mRect.width > 30 && mRect.height > 30) {
            ...
            if (hasModal === 1 && currentModalProgress > 0.05) {
            ...
            // 2. 探测所有 Layer 2 液态透镜 (状态变更首帧即时捕获，动画期间保持锁定杜绝跳帧)
            ...
            if (hasModal !== lastModalState || (!isAnimatingModal && lensScanFrameCounter % 15 === 0)) {
                ...
                cachedLensElements = Array.from(document.querySelectorAll('[data-composer-card], [class*="InputTrigger"], [class*="ChatInput"], [data-dsh-inputbar] > div, button[class*="newSession"], [class*="SidebarRoot_groupSection"], [class*="sidebarCol"] [class*="groupSection"], [class*="SegmentedControl_item"], [class*="themeCard"], [class*="navCell"][class*="active"], [class*="AppearanceChoice"]'));
            }
            let count = 0;
            lensBuffer.fill(0);
            radiiBuffer.fill(0);
            ...
            for (let i = 0; i < cachedLensElements.length && count < 64; i++) {
                ...
                if (!el || (el.offsetWidth === 0 && el.offsetHeight === 0))
                    continue;
                // 绝不扫描插件中心、市场、技能卡片，保持插件列表轻量纯净
                if (el.closest('[class*="Plugin"], [class*="plugin"], [class*="Marketplace"], [class*="marketplace"], [class*="Skill"], [class*="skill"]') !== null) {
                    continue;
                }
                const rect = el.getBoundingClientRect();
                if (rect.width < 14 || rect.height < 14 || rect.bottom <= 0 || rect.top >= screenH)
                    continue;
                ...
                if (isInsideSidebar && !isInsideModal) {
                    ...
                    if (isWorkspaceCard && effectiveW < 130)
                        continue;
                    if (isNewSessionBtn && effectiveW < 32)
                        continue;
                }
                let left = rect.left;
...
//# sourceMappingURL=glass-shader.js.map