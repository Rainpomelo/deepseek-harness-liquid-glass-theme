...
html[data-dsh-liquid-glass] .dsh-segmented-slider-track.dsh-is-dragging,
html[data-dsh-liquid-glass] [class*="segmentedSliderTrack"][class*="dragging"] {
...
html[data-dsh-liquid-glass] .dsh-segmented-slider-track.dsh-is-dragging .dsh-segmented-sliding-indicator {
  transition: none !important;
  transform: scale(1.02) !important;
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.60), 0 4px 14px rgba(0, 0, 0, 0.40) !important;
}
...
`
  ...
  const updateDomPluginCard = () => {
    const card = document.querySelector<HTMLElement>('[data-dsh-liquid-glass-card="true"]')
    if (!card) return
    const isEnabled = layer.getEnabled()
    const switchBtn = card.querySelector<HTMLElement>('.dsh-liquid-glass-switch, button[class*="toggle"]')
    if (switchBtn) {
      ...
      const track = switchBtn.querySelector<HTMLElement>('.dsh-switch-track, [class*="switchTrack"]')
      if (track) {
        track.style.background = isEnabled ? '#38bdf8' : 'rgba(0, 0, 0, 0.40)'
        track.style.borderColor = isEnabled ? '#7dd3fc' : 'rgba(255, 255, 255, 0.15)'
      }
      const thumb = switchBtn.querySelector<HTMLElement>('.dsh-switch-thumb, [class*="switchThumb"]')
      if (thumb) {
        thumb.style.transform = isEnabled ? 'translateX(12px)' : 'translateX(0)'
      }
      const label = switchBtn.querySelector<HTMLElement>('[class*="switchLabel"], .dsh-switch-text')
      if (label) {
  ...
  const injectPluginCardIfMissing = () => {
    ...
    const cardLists = document.querySelectorAll<HTMLElement>(
    ...
    for (const list of cardLists) {
      ...
      // 绑定快捷开关事件
      const switchBtn = li.querySelector<HTMLElement>('.dsh-liquid-glass-switch')
      if (switchBtn) {
      ...
      // 绑定跳转事件
      const jumpBtn = li.querySelector<HTMLElement>('.dsh-liquid-glass-jump-btn')
      if (jumpBtn) {
  ...
  // 监听 DOM 变动，确保在切换到设置 -> 插件页面时快捷开关始终存在
